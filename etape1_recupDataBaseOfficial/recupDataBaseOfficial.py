...
fichier (voir main.py pour ça) — une seule fonction d'entrée qui prend en
paramètres les mots-clés / départements / options de recherche, et renvoie
une structure de données Python (liste de dict, directement sérialisable en
...
# =====================================================================
# Constantes partagées (réutilisées aussi comme valeurs par défaut par main.py)
# =====================================================================
...
def _construire_where_boamp(mots_cles: list[str], departements: list[str], seulement_ouverts: bool) -> str:
    """Construit la clause ODSQL `where=` : (motclés OR) AND (départements OR) AND (date limite).
    ...
    clauses = [f"({clause_motcles})", f"({clause_departements})"]
...
def interroger_boamp(
    ...
    where = _construire_where_boamp(mots_cles, departements, seulement_ouverts)
    if verbeux:
...
def _construire_requete_ted(mots_cles: list[str], departements: list[str], seulement_ouverts: bool) -> str:
    """Construit la requête "expert query" TED : (motclés OR) AND (NUTS OR) AND (deadline)."""
    ...
    clauses = [f"({clause_motcles})", f"({clause_nuts})"]
...
def interroger_ted(
    ...
    query = _construire_requete_ted(mots_cles, departements, seulement_ouverts)
    if verbeux:
...
def recuperer_appels_offres(
    ...
    if sources.get("boamp"):
        records_boamp = interroger_boamp(mots_cles, departements, seulement_ouverts, limit_par_source, verbeux)
        tous_resultats.extend(normaliser_boamp(records_boamp))

    if sources.get("ted"):
        notices_ted = interroger_ted(mots_cles, departements, seulement_ouverts, limit_par_source, verbeux)
